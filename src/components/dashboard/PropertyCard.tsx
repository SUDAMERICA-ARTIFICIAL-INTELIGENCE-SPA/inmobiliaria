...
function getStatusLabel(status: string): { label: string; className: string } {
    ...
    if (s.includes("sold")) return { label: "VENDIDO", className: "bg-red-500/80 text-white" };
    if (s.includes("pending")) return { label: "PENDIENTE", className: "bg-yellow-500/80 text-black" };
    return { label: "EN VENTA", className: "bg-[#00F0FF]/80 text-black" };
}
...
export function PropertyCard({ property, onAnalyzeOwner, onHover }: PropertyCardProps) {
    ...
    return (
        <motion.div
            ...
            className="h-full"
        >
            <Card className="glass-card overflow-hidden h-full flex flex-col group border-transparent hover:border-[#00F0FF]/50 transition-colors duration-300 cursor-pointer"
                ...
                <div className="p-4 flex flex-col flex-grow">
                    ...
                    <div className="flex items-center gap-3 text-xs text-gray-400 mb-4 whitespace-nowrap overflow-hidden">
                        ...
                        <div className="flex items-center gap-1">
                            ...
                            <span>{property.baths_full + (property.baths_half * 0.5)}</span>
                        </div>
                        <div className="w-px h-3 bg-white/10" />
                        <div className="flex items-center gap-1">
